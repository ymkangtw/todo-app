...
- **Database**: Supabase PostgreSQL via Sequelize (raw SQL queries, no ORM models). Connection config in `server/.env` (`DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`). Uses Supavisor session mode (port 5432)
...
- **Layer structure**:
  ...
  - `ctrl/todos.ctrl.js` - Async business logic using `sequelize.query()` with named replacements and `withTransaction`
  - `models/db.js` - Sequelize instance (no model definitions), `QueryTypes`, async `withTransaction` helper
